...
class BedrockKBProcessor:
    ...
    def process_file(self, file: IO[bytes], extension: str, file_date: str) -> list[RFPChunk]:
        rfp_chunks: list[RFPChunk] = []
        data: list[str] = self.__extract_data_concurrent(file, extension)
    ...
    def __extract_data_concurrent(self, file: IO[bytes], extension: str):
        sections = self.__load_file_in_sections(file, extension)
        data: list[str] = []
    ...
    def __load_file_in_sections(self, file: IO[bytes], extension: str) -> list[pd.DataFrame]:
        ...
        if extension.upper() == "XLSX":
            ...
            sheets_dict = pd.read_excel(
                file, sheet_name=None, header=None, engine="openpyxl"
            )

            sections = list(sheets_dict.values())

        else:
            logger.info("Loaded CSV")
            sheet = pd.read_csv(file, header=0, sep=self.separator)
            sections = [sheet]
